...
T5Decoding<T>::T5Decoding(T5Decoding<T> const& decoding):
    ...
    local_head_num_(decoding.local_head_num_),
    ...
    for (int step = max_input_length; step <= (int)max_seq_len; step++) {
        ...
        for (uint ite = 0; ite < iteration_num; ++ite) {
            ...
            std::vector<Tensor> decoder_input_tensors{
                ...
                Tensor{MEMORY_GPU,
                       data_type,
                       decoding_weights->position_embedding_type == PositionEmbeddingType::linear ? std::vector<size_t>{local_head_num_} : std::vector<size_t>{1, head_num_, max_seq_len + 1, max_seq_len + 1},
                       decoding_weights->position_embedding_type == PositionEmbeddingType::linear ? linear_bias_slopes_ + local_head_num_ * tensor_para_.rank_ : decoding_weights->position_embedding_type == PositionEmbeddingType::relative ?
